...
"""
Distribution metrics by https://github.com/eth-ait/motion-transformer/blob/master/metrics/distribution_metrics.py from the paper
'A Spatio-temporal Transformer for 3D Human Motion Prediction' by E. Aksan et al.
"""
...
def power_spectrum(seq):
    ...
    dims_to_use = np.where(
        (np.reshape(seq_t, [-1, n_joints, feature_size]).std(0) >= 1e-4).all(axis=-1)
    )[0]
